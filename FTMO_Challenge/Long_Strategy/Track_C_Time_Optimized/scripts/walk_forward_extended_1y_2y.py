"""
Extended Walk-Forward Validation for Track C (1yr train / 2yr test, 4 folds).

This script performs extended rigor validation on Track C's optimized parameters.
Uses the same configuration as Track B for direct comparison.
"""
...
def score_train(metrics: Dict) -> float:
    """Compliance-first scoring for Track C.

    Primary objective: stay within FTMO limits.
    Secondary objective: maximize return quality.
    """
    compliant = metrics["max_drawdown_pct"] <= 10.0 and metrics["worst_daily_loss_pct"] >= -5.0
    score = 10000.0 if compliant else -10000.0

    # Heavy penalties for limit breaches.
    if metrics["max_drawdown_pct"] > 10.0:
        score -= (metrics["max_drawdown_pct"] - 10.0) * 80.0
    if metrics["worst_daily_loss_pct"] < -5.0:
        score -= abs(metrics["worst_daily_loss_pct"] + 5.0) * 120.0

    # Within compliant space, prefer robust positive return profile.
    score += metrics["return_pct"] * 2.0
    score += min(metrics["profit_factor"], 3.0) * 8.0
    score += min(metrics["win_rate"], 60.0) * 0.2
    return score


def param_grid() -> List[Params]:
    """Generate parameter search grid (drawdown-aware, aligned with Track B)."""
    grid: List[Params] = []
    fasts = [8, 10, 12]
    slows = [20, 26, 50]
    sls = [1.0, 1.25]
    tps = [3.0, 4.0]
    risks = [0.2, 0.25, 0.3, 0.35]
...
def run_walk_forward_extended(
    ...
    step_bars: int = 24 * 365 * 2,      # 2 year step (non-overlapping test windows)
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="FTMO_Challenge/Long_Strategy/Track_C_Time_Optimized/reports",
        help="Output directory for results",
    )
    parser.add_argument(
        "--track",
        default="Track_C_Extended",
    ...
    print(f"Track C - Time Optimized")
    print(f"{'='*60}")
    ...
    folds_df, oos_trades_df, summary = run_walk_forward_extended(
        ...
        step_bars=24 * 365 * 2,      # 2 year step (non-overlapping test windows)
